        ...
        <tr>
            ...
            <td>
                <div class="container" style="display: flex;">
                    ...
                    <a class="btn btn-warning m-2" id="{{$offre->id}}" href="{{route('signalOffre', ['offre' => $offre->id])}}/">Signaler</a>
                    @else
                    <a class="btn btn-danger m-2" id="{{$offre->id}}" href="{{route('unsignalOffre', ['offre' => $offre->id])}}">Désignaler</a>
                    @endif
                    <form method="POST" action="{{route('destroyOffre', ['offre' => $offre->id])}}" style="margin: 0px">
                        @csrf
                        @method('DELETE')
        ...
        <tr>
            ...
            <td>
                <div id="{{$demande->id}}" class="container" style="display: flex;">
                    <a class="btn btn-primary m-2" href="{{route('show.demandes', ['demande' => $demande->id])}}">Voir</a>
                    @if ($demande->etatDemande == 'Valide')
                    <a class="btn btn-warning m-2"  href="{{route('signalDemande', ['demande' => $demande->id])}}/">Signaler</a>
                    @else
                    <a class="btn btn-danger m-2"  href="{{route('unsignalDemande', ['demande' => $demande->id])}}/">Désignaler</a>
                    @endif
                    <form method="POST" action="{{route('destroyDemande', ['demande' => $demande->id])}}" style="margin: 0px">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger m-2">Supprimer</button>
                    </form>                        
